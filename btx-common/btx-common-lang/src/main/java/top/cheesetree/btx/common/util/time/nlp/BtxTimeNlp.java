...
import top.cheesetree.btx.common.util.comm.AllEnums;
...
/**
 * @author van
 * @date 2023/5/23 09:04
 * @description TODO
 */
@Getter
@Setter
public class BtxTimeNlp {
     ...
     */
    public BtxTimeNlp(String timeExpression, TextAnalysis textAnalysis) {
    ...
    public BtxTimeNlp(String timeExpression, TextAnalysis textAnalysis, TimeContext timePoint) {
        this.timeExpression = timeExpression;
        this.textAnalysis = textAnalysis;
    ...
    private void normCurRelated() {
        ...
        boolean isDay = false;
        pattern = AllEnums.TimeNlpRegexEnum.NormCurRelatedDayBeforeAny.getPattern();
        match = pattern.matcher(timeExpression);
        if (match.find()) {
            flag[2] = true;

            match = AllEnums.TimeNlpRegexEnum.NormCurRelatedDayAny.getPattern().matcher(timeExpression);
            int day = 0;
            while (match.find()) {
                day++;
            }
        ...
        pattern = AllEnums.TimeNlpRegexEnum.NormCurRelatedDayAfterAny.getPattern();
        match = pattern.matcher(timeExpression);
        if (match.find()) {
            flag[2] = true;
        ...
        if (match.find()) {
            ...
            int week;
            try {
                week = Integer.parseInt(timeExpression.substring(timeExpression.length() - 1));
            } catch (NumberFormatException e) {
                week = 1;
                isWeek = true;
            }

            match = AllEnums.TimeNlpRegexEnum.NormCurRelatedBeforeAny.getPattern().matcher(timeExpression);
        ...
        if (match.find()) {
            ...
            match = AllEnums.TimeNlpRegexEnum.NormCurRelatedAfterAny.getPattern().matcher(timeExpression);
            int nweek = 0;
            while (match.find()) {
                nweek++;
            }
     ...
     * @return 返回结果
     */
    public static List<BtxTimeNlp> filterTimeUnit(List<BtxTimeNlp> timeNLPList) {
        if (CollectionUtil.isEmpty(timeNLPList)) {
            return timeNLPList;
        }
        List<BtxTimeNlp> list = new ArrayList<>();
        for (BtxTimeNlp t : timeNLPList) {
